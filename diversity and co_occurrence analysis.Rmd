...
Diversity analysis
```{r}
sugs<-c("Ac","FucNAc" ,'Gal',"GalNAc",'Glc', "GlcA","GlcNAc",   "Gro","ManNAc",'Rha','any.nac')
...
```{r}
reg.array<-abind(cc.div, mlst.div, GPSC.div,phylo.div100,phylo.div200,phylo.div300, along=3)
dimnames(reg.array)[[3]]<-c('cc','mlst','gpsc', 'phylo100', 'phylo200', 'phylo300')
reg.array<-reg.array[order(reg.array[,'AIC','cc']),,]
plot(reg.array[1:dim(reg.array)[1],'estimate','cc'], pch=16, col='#e41a1c', bty='l', ylim=range(reg.array[,c("estimate", "2.5 %","97.5 %"),]), ylab='Coefficient', xaxt='n', xlab='')
...
points(((1:dim(reg.array)[1]+0.3)),reg.array[,'estimate','phylo100'], pch=18, col='#984ea3')
points(((1:dim(reg.array)[1]+0.4)),reg.array[,'estimate','phylo200'], pch=18, col='#ff7f00')
points(((1:dim(reg.array)[1]+0.5)),reg.array[,'estimate','phylo300'], pch=18, col='#a65628')
...
arrows(x0=((1:dim(reg.array)[1]+0.3)),y0=reg.array[,"2.5 %",'phylo100'],y1=reg.array[,"97.5 %",'phylo100'], pch=16, col='#984ea3', length=0)
arrows(x0=((1:dim(reg.array)[1]+0.4)),y0=reg.array[,"2.5 %",'phylo200'],y1=reg.array[,"97.5 %",'phylo200'], pch=16, col='#ff7f00', length=0)
arrows(x0=((1:dim(reg.array)[1]+0.5)),y0=reg.array[,"2.5 %",'phylo300'],y1=reg.array[,"97.5 %",'phylo300'], pch=16, col='#a65628', length=0)

abline(h=0, lty=2, col='gray')
text(1:11, y=-2.5, dimnames(reg.array)[[1]], pos=4, offset=0, xpd=NA, col='gray', cex=0.75)
#legend("topright", legend=c("CC", "MLST", "GPSC", 'Phylo100', 'Phylo200', 'Phylo300'),       col=c("#e41a1c", "#377eb8", "#4daf4a",'#984ea3','#ff7f00','#a65628'), pch=c(16,17,18), box.lty=0)
...
points(((1:dim(plot.array)[1]+0.3)),plot.array[,'estimate','phylo200'], pch=18, col='#ff7f00')
points(((1:dim(plot.array)[1]+0.4)),plot.array[,'estimate','phylo300'], pch=18, col='#a65628')
...
arrows(x0=((1:dim(plot.array)[1]+0.3)),y0=plot.array[,"2.5 %",'phylo200'],y1=plot.array[,"97.5 %",'phylo200'], pch=16, col='#ff7f00', length=0)
arrows(x0=((1:dim(plot.array)[1]+0.4)),y0=plot.array[,"2.5 %",'phylo300'],y1=plot.array[,"97.5 %",'phylo300'], pch=16, col='#a65628', length=0)



abline(h=0, lty=2, col='gray')
text(1:10, y=-1, c('Glc','GlcA','Any NAc', 'Rha','Gal','GlcNAc','Gro','FucNAc', 'Rib-ol','ManNAc'), pos=4, offset=0, xpd=NA, col='gray', cex=0.75, srt=90)
legend(1.1,3, legend=c("CC", "MLST", "GPSC", 'Phylo200', 'Phylo300'),
       col=c("#e41a1c", "#377eb8", "#4daf4a","#ff7f00","#a65628"), pch=c(16,17,18, 18,18), box.lty=0, adj=c(0.1, 0.5))
}
```
