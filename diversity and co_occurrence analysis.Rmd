...
all.sdi<- merge(GPSC.sdi, mlst.sdi, by='st')
all.sdi<-merge(all.sdi, cc.sdi, by='st')
all.sdi<-all.sdi[order(all.sdi$st.cc.diversity),]
#Plot the CC Diversity

#MLST vs CC
set.seed(123)
par(mfrow=c(1,2))
symbols(all.sdi$st.cc.diversity, all.sdi$st.cc.diversity.y,circles=sqrt(all.sdi$st.cc.sum+all.sdi$st.cc.sum.y), inches=0.25, bty='l', xlab='Serotype Diversity (CC)' ,ylab='Serotype Diversity (MLST)', fg='grey')
text(all.sdi$st.cc.diversity, (all.sdi$st.cc.diversity.y+rnorm(mean=0,sd=0.02, n=nrow(all.sdi))), all.sdi$st, col='black', cex=0.5, adj=c(1,1))
#CC vs GPSC
symbols(all.sdi$st.cc.diversity, all.sdi$st.cc.diversity.x,circles=sqrt(all.sdi$st.cc.sum+all.sdi$st.cc.sum.x), inches=0.25, bty='l', xlab='Serotype Diversity (CC)' ,ylab='Serotype Diversity (GPSC)', fg='grey')
text(all.sdi$st.cc.diversity, (all.sdi$st.cc.diversity.x+rnorm(mean=0,sd=0.02, n=nrow(all.sdi))), all.sdi$st, col='black', cex=0.5, adj=c(1,1))

#CC
p1<-ggplot(data=all.sdi, aes(x=reorder(st,-st.cc.diversity), y=st.cc.diversity)) +
  geom_point(alpha = 0.5, aes(size = st.cc.sum)) + labs(x = "Serotype", y='SDI CC')+
coord_flip() + theme(panel.background = element_blank(), axis.line = element_line(size = 0.5), axis.text = element_text(face = "bold")) +
  theme(legend.position = "none")
#rownames(sdi_barplot) <- sdi_barplot$Serotype

#MLST
p3<-ggplot(data=all.sdi, aes(x=reorder(st,-st.cc.diversity), y=st.cc.diversity.y)) +
  geom_point(alpha = 0.5, aes(size = st.cc.sum)) + labs(x = "", y='SDI MLST')+
  coord_flip() + theme(panel.background = element_blank(), axis.line = element_line(size = 0.5), axis.text = element_text(face = "bold")) +
  theme(legend.position = "none")
#rownames(sdi_barplot) <- sdi_barplot$Serotype
...
#GPSC
p2<-ggplot(data=all.sdi, aes(x=reorder(st,-st.cc.diversity), y=st.cc.diversity.x)) +
  geom_point(alpha = 0.5, aes(size = st.cc.sum)) + labs(x = "", y='SDI GPSC')+
 coord_flip() + theme(panel.background = element_blank(), axis.line = element_line(size = 0.5), axis.text = element_text(face = "bold")) +
  theme(legend.position = "none")
#rownames(sdi_barplot) <- sdi_barplot$Serotype
...
#plot(x=all.sdi$st.cc.diversity, y=1:nrow(all.sdi), xlim=c(0,1), col='red', pch=16)
#points(x=all.sdi$st.cc.diversity.x, y=1:nrow(all.sdi), col='blue',pch=17)
#points(x=all.sdi$st.cc.diversity.y, y=1:nrow(all.sdi), col='black', pch=18)
#plot(all.sdi$st.cc.diversity, all.sdi$st.cc.diversity.x)
#plot(all.sdi$st.cc.diversity, all.sdi$st.cc.diversity.y)
...
text(1:10, y=-2.5, dimnames(reg.array)[[1]], pos=4, offset=0, xpd=NA, col='gray', cex=0.75)
...
plot(plot.array[1:dim(plot.array)[1],'estimate','cc'], pch=16, col='#e41a1c', bty='l', ylim=c(-1,2), ylab=paste0(model.labs[i],' Coefficient'), xaxt='n', xlab='')
points(((1:dim(plot.array)[1]+0.15)),plot.array[,'estimate','mlst'], pch=17, col='#377eb8')
points(((1:dim(plot.array)[1]+0.3)),plot.array[,'estimate','gps'], pch=18, col='#4daf4a')

arrows(x0=1:dim(plot.array)[1],y0=plot.array[,"2.5 %",'cc'],y1=plot.array[,"97.5 %",'cc'], pch=16, col='#e41a1c', length=0)
arrows(x0=((1:dim(plot.array)[1]+0.15)),y0=plot.array[,"2.5 %",'mlst'],y1=plot.array[,"97.5 %",'mlst'], pch=16, col='#377eb8', length=0)
arrows(x0=((1:dim(plot.array)[1]+0.3)),y0=plot.array[,"2.5 %",'gps'],y1=plot.array[,"97.5 %",'gps'], pch=16, col='#4daf4a', length=0)
