...
##Set threshold for minimum number of isolates on a CC or MLST
```{r}
min.n.isolates<-5
```
...
Clonal cluster
```{r}
cc1.original<-read.csv('./Data/serotype_cluster_mat.csv')
names(cc1.original)[1]<-'st'
cc1.original1<-as.matrix(cc1.original[,-1])
cc1.original1[cc1.original1<min.n.isolates]<-0
cc1<-cbind.data.frame('st'=cc1.original$st,cc1.original1)
```

MLST
```{r}
mlst1.original<-read.csv('./Data/serotype_mlst_mat.csv')
names(mlst1.original)[1]<-'st'
mlst1.original1<-as.matrix(mlst1.original[,-1])
mlst1.original1[mlst1.original1<min.n.isolates]<-0
mlst1<-cbind.data.frame('st'=mlst1.original$st,mlst1.original1)
...
#GPSC
p2<-ggplot(data=all.sdi, aes(x=reorder(st,-st.cc.diversity.x), y=st.cc.diversity.x)) +
...
#MLST
p3<-ggplot(data=all.sdi, aes(x=reorder(st,-st.cc.diversity.y), y=st.cc.diversity.y)) +
...
```{r}
combined.tab<-cbind( cc.tab,gps.tab)
